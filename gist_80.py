    ...
    def _fn(*args, **kwargs):
        try:
            ...
            user_settings_path = os.path.join(
                sublime.packages_path(), 'User', 'Gist.sublime-settings'
            )
            if not os.path.exists(user_settings_path):
                default_settings_path = os.path.join(
                    sublime.packages_path(), 'Gist', 'Gist.sublime-settings'
                )
...
def create_gist(public, description, files):
    ...
    file_data = dict(
        (filename, {'content': text}) for filename, text in list(files.items())
    )
    data = json.dumps(
        {'description': description, 'public': public, 'files': file_data}
    )
...
def update_gist(
    gist_url, file_changes, auth_token=None, https_proxy=None, new_description=None
):
    ...
    result = api_request(
        gist_url, data, token=auth_token, https_proxy=https_proxy, method="PATCH"
    )
...
def open_gist(gist_url):
    ...
    for gist_filename in files:
        ...
        if settings.get('update_on_save'):
            view.retarget(
                os.path.join(
                    tempfile.gettempdir(), "{}-{}".format(gist['id'], gist_filename)
                )
            )
    ...
    def run(self, edit):
        ...
        def on_gist_description(description):
                ...
                if len(region_data) == 1:
                    ...
                    if filename:
                        ...
                        syntax_name, _ = os.path.splitext(
                            os.path.basename(self.view.settings().get('syntax'))
                        )
            ...
            window.show_input_panel(
                'Gist File Name: (optional):', filename, on_gist_filename, None, None
            )

        window.show_input_panel(
            "Gist Description (optional):", '', on_gist_description, None, None
        )
...
class GistRenameFileCommand(GistViewCommand, sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        self.view.window().show_input_panel(
            'New File Name:', old_filename, on_filename, None, None
        )
                ...
                for window in sublime.windows():
                    for view in window.views():
                        if view.settings().get('gist_url') == gist_url:
                            gistify_view(
                                view, new_gist, view.settings().get('gist_filename')
                            )
                sublime.status_message('Gist description changed')

        self.view.window().show_input_panel(
            'New Description:',
            self.gist_description() or '',
            on_gist_description,
            None,
            None,
        )
    ...
    def run(
        ...
        if settings.get('include_orgs'):
            if settings.get('include_orgs') is True:
                self.orgs = [
                    org.get("login") for org in api_request(settings.get('ORGS_URL'))
                ]
            else:
        ...
        def on_gist_num(num):
            ...
            if num < 0:
                ...
                filtered = gists_filter(
                    api_request(
                        settings.get('USER_GISTS_URL') % self.users[num - off_orgs]
                    )
                )
                self.gists = filtered[0]
                gist_names = filtered[1]
...
class GistAddFileCommand(GistListCommandBase, sublime_plugin.TextCommand):
    ...
    def handle_gist(self, gist):
        ...
        self.view.window().show_input_panel(
            'File Name:', filename, on_filename, None, None
        )
